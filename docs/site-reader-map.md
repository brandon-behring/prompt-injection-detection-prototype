...
- **Full methodology read** (pick a register): [`WRITEUP_PAPER`](../WRITEUP_PAPER.md) is the academic IMRAD version (~20–25 min); [`WRITEUP_NARRATIVE`](../WRITEUP_NARRATIVE.md) is the narrative-arc version (~15–20 min); [`WRITEUP`](../WRITEUP.md) is a 1-page router that points at the two guides.
