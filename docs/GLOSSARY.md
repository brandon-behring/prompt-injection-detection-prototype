...
Older ADRs (ADR-007 onwards) refer to the evaluated classifier approaches as **rungs** (decision-tree language from the spec-lock-in conversations). Reader-facing prose in WRITEUP_PAPER / WRITEUP_NARRATIVE / RESULTS / README calls them **detectors** (a more familiar noun for someone not steeped in the SDD vocabulary). Both terms refer to the same thing: the **5 evaluated approaches in the headline ladder** — TF-IDF + LR (classical floor); ModernBERT frozen-probe; ModernBERT LoRA; ModernBERT full-FT (LODO direct-source only — pooled OOD inference dropped at Phase 5 X11 per ADR-075); and the ProtectAI v1 + v2 reference scorers (counted as one rung-tier per ADR-005 + ADR-018; reported separately for slice-dependence). The LLM-judge tier (gpt-4o + claude-sonnet) was locked at Phase 0-03 per ADR-018 and DROPPED at Phase 4 cost re-estimation per ADR-050 (16× envelope overrun); it does not appear in the headline ladder.
