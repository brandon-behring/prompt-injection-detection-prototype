...
A single locked decision in Michael Nygard format: Status / Context / Decision / Consequences / Alternatives Considered. **Immutable**, with two narrow exceptions (see [Immutability relaxation — two factual-defect classes](#immutability-relaxation--two-factual-defect-classes) below). See `decisions/README.md` for the lifecycle + `decisions/ADR_TEMPLATE.md` for the schema.

## Immutability relaxation — two factual-defect classes

Per [ADR-067](../decisions/ADR-067-immutability-clarification-and-canonical-slug-reference.md) (added at v1.2.2) + [ADR-068](../decisions/ADR-068-immutability-narrow-relaxation-for-broken-external-references.md) (added at v1.2.5): the ADR-immutability rule (CLAUDE.md: *"ADRs are immutable; supersede via new ADR"*) has TWO narrow exceptions covering distinct factual-defect classes:
...
Both classes MAY be corrected in-place with a commit message citing the relevant ADR + listing per-file corrections. ALL other content (numeric values, methodology decisions, prose, alternatives considered, non-slug frontmatter fields, table content) remains immutable per the existing rule — change requires a superseding ADR. The narrow exceptions do NOT establish a slippery slope; ADR-067 §B + ADR-068 §B explicitly enumerate in-scope vs out-of-scope changes for each class.
