...
- **SDD / ADR process**: 77 ADRs accepted across Phase 0-00 through ADR-077 (frontmatter-backfill + supersession-backlink invariant governance closures); each significant decision locked before code; `SUBMISSION_AUDIT.md` regenerates from ADR frontmatter via `scripts/regenerate_audit.py` (CI hard-gate per ADR-039).
