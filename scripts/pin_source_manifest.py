"""Live-fetch HF + GitHub SHAs and write configs/data/source_manifest.yaml (per ADR-041 Q2; relocated per ADR-044 Q2).
...
`git ls-remote`. Output: `configs/data/source_manifest.yaml` (rich schema per
ADR-041 Q1; 13 fields per source row; schema_version 1.0; `bump_history: []`).
...
def _write_manifest(
    ...
    with manifest_path.open("w", encoding="utf-8") as fh:
        fh.write(
            "# configs/data/source_manifest.yaml — Phase 1 source pin per ADR-016 Q3 + ADR-041 Q1 + ADR-044 Q2.\n"
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=_REPO_ROOT / "configs" / "data" / "source_manifest.yaml",
        help="Output manifest path (default: configs/data/source_manifest.yaml)",
    )
