...
2. **ADR slug references**: every `decisions/ADR-NNN-<slug>.md` link target verified against actual filenames in `decisions/`. Catches the broken-slug class lychee covers AND semantically-wrong-but-existing slug refs (e.g., "ADR-029 immutability" pointing at an existing ADR-029 file that is actually about test markers — the historical drift documented in project-local Claude memory `memory/adr-029-immutability-misattribution-historical-drift.md` at the meta-level; not committed to the repo).
