...
|---|---|---|
| `eval-toolkit` | `v0.47.0` | `eval-toolkit==0.47.0` **(PyPI install per ADR-055 narrow supersession of ADR-036)**; bumped v0.31→v0.34 mid-Phase-4 X8 commit `8af73ae`; v0.34→v0.39 at v1.0.6 (consumed #39 `is_metric_defined_for_slice` + #40 `LeakageCheck.name` + #41 `parallel_map`); v0.39→v0.40 + git+https→PyPI at v1.0.8 (consumed #43 `fit_platt_binary` + `fit_beta_binary`; refactored to `_binary` calibrator family per ADR-056; filed #44 `fit_isotonic_binary` for shape consistency); v0.40→v0.42 at v1.0.9 (consumed #44 `fit_isotonic_binary` + removed `fit_isotonic_binary_local` adapter; v0.41 skipped as predated #44 close); v0.42→v0.43 at v1.2.8 (closes #48 `ood_dataset_from_manifest`, #49 `character_injection` core-6, #53 `ActivationDeltaProbe`; dependency/ledger maintenance only, no methodology/model/data change); v0.43→v0.44 at v1.2.14 (closes #50 `RecallAtLowFPR` Meta Prompt Guard 2 loss recipe + #51 `spotlighting` defense — delimit/datamark/encode + sweep helpers; new `[losses]` extra with optional `torch>=2.0` dep; spotlighting is base-install safe — pure stdlib. Dependency/ledger maintenance bump per ADR-066 trigger #4; no consumer-side code change in this project's evaluation slate.); v0.44→v0.47 at v1.2.16 (skip-bumps 3 minors: closes #52 `LogisticStacker` + `MetaLearner` Protocol in v0.45.0; closes #36 inline CI via `scorecard()` + `Scorecard` + `metric_specs` in v0.46.0; closes #49 advanced-6 `character_injection` + sweep unification in v0.47.0. v0.46 soft-deprecated top-level scalar metric imports via `__getattr__` shim; v0.47 hard-removed the shim + per-module strategy Protocols + `character_injection`/`spotlighting` SimpleNamespaces. Our project uses only submodule imports (`from eval_toolkit.metrics import pr_auc`, `from eval_toolkit.bootstrap import bootstrap_ci`, etc.) which are preserved through the v0.46+v0.47 breaking-change cycle. Dependency/ledger maintenance bump per ADR-066 trigger #4; no consumer-side code change; full gate (7 audit scripts + pre-commit + pytest smoke 179 passed) green pre-commit.) |
