...
|---|---|---|
| `eval-toolkit` | `v1.0.3` (range `>=1.0.3,<2`) | `eval-toolkit>=1.0.3,<2` **(PyPI install per ADR-055; range pin per v1.0 stability-contract; v1.3.8 tightened lower bound to require module `eval_toolkit.audit_value_bindings` shipped in v1.0.3)**. Trajectory: v0.31→v0.34 mid-Phase-4 X8; v0.34→v0.39 at v1.0.6; v0.39→v0.40 + git+https→PyPI at v1.0.8 (filed #44); v0.40→v0.42 at v1.0.9 (#44); v0.42→v0.43 at v1.2.8 (ledger only); v0.43→v0.44 at v1.2.14 (#50+#51); v0.44→v0.47 at v1.2.16 (skip-bumps; ledger); v0.47→v0.49→v0.50 at v1.3.5; v0.50→v1.0 at v1.3.6 (stability-contract opt-in); v1.0→v1.0.1→v1.0.2 at v1.3.7 (**consumed `eval_toolkit.audit_citation_alignment` — closes #73**); v1.0.2→v1.0.3 at v1.3.8 (**consumed `eval_toolkit.audit_value_bindings` — flat module per upstream ADR 0001; closes #71**; second member of audit-validator family). See per-CHANGELOG entries + `decisions/upstream_issues.md` for full per-issue closure narratives. |
