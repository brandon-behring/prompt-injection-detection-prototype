...
|---|---|---|
| `eval-toolkit` | `v1.0.0` (range `>=1.0,<2`) | `eval-toolkit>=1.0,<2` **(PyPI install per ADR-055 narrow supersession of ADR-036; range pin per the v1.0 stability-contract opt-in at v1.3.6 — upstream ADR 0003 Tier 1 STRICT signatures now load-bearing; uv.lock provides byte-level reproducibility on top)**; bumped v0.31→v0.34 mid-Phase-4 X8 commit `8af73ae`; v0.34→v0.39 at v1.0.6 (consumed #39 + #40 + #41); v0.39→v0.40 + git+https→PyPI at v1.0.8 (consumed #43; filed #44); v0.40→v0.42 at v1.0.9 (consumed #44; v0.41 skipped); v0.42→v0.43 at v1.2.8 (closes #48 + #49 core-6 + #53; dependency/ledger maintenance only); v0.43→v0.44 at v1.2.14 (closes #50 + #51; dependency/ledger maintenance per ADR-066 trigger #4); v0.44→v0.47 at v1.2.16 (skip-bumps 3 minors: closes #52 in v0.45.0; closes #36 via `scorecard()`+`Scorecard`+`metric_specs` in v0.46.0; closes #49 advanced-6 + sweep unification in v0.47.0. v0.47.0 BREAKING removals don't hit our project — we use only submodule imports which are preserved. Dependency/ledger maintenance only); v0.47→v0.49→v0.50 at v1.3.5 (Global naming-standards sweep at v0.49; SPEC 7 `rng=` parameter adoption at v0.50 — `paired_bootstrap_diff` `seed=`→`rng=` consumer migration at commit `da471d5`); v0.50→v1.0 at v1.3.6 (stability-contract opt-in; v1.0 is bit-equivalent to v0.51 codewise — Round 8 + Round 9 + Round 10 multi-LLM audit rectification batch — but the new thing at v1.0 is that Tier 1 STRICT public-API signatures in `tests/golden/public_api/snapshot.json` become load-bearing per ADR 0003; breaking changes after v1.0 require a v2.0 major bump. None of the 4 v0.51 BREAKING changes hit consumer callsites — verified at v1.3.6 close per CHANGELOG `Verified` section). See per-CHANGELOG entries + `decisions/upstream_issues.md` for full per-issue closure narratives. |
