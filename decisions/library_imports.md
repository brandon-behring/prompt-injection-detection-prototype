...
|---|---|---|
| `eval-toolkit` | `v1.0.0` (range `>=1.0,<2`) | `eval-toolkit>=1.0,<2` **(PyPI install per ADR-055; range pin per the v1.0 stability-contract opt-in at v1.3.6 — upstream ADR 0003 Tier 1 STRICT signatures load-bearing; uv.lock provides byte-level reproducibility)**. Trajectory: v0.31→v0.34 mid-Phase-4 X8 commit `8af73ae`; v0.34→v0.39 at v1.0.6 (#39+#40+#41); v0.39→v0.40 + git+https→PyPI at v1.0.8 (#43; filed #44); v0.40→v0.42 at v1.0.9 (#44; v0.41 skipped); v0.42→v0.43 at v1.2.8 (#48+#49 core-6+#53; ledger only); v0.43→v0.44 at v1.2.14 (#50+#51; ledger per ADR-066 trigger #4); v0.44→v0.47 at v1.2.16 (skip-bumps 3 minors closing #52/#36/#49 advanced-6; v0.47.0 BREAKING removals miss our submodule-only imports; ledger only); v0.47→v0.49→v0.50 at v1.3.5 (v0.49 naming sweep; v0.50 SPEC 7 `rng=` adoption — `paired_bootstrap_diff` `seed=`→`rng=` consumer migration at commit `da471d5`); v0.50→v1.0 at v1.3.6 (stability-contract opt-in; v1.0 = v0.51 codewise + ADR 0003 Tier 1 STRICT signature freeze; 4 BREAKING + 1 deprecation verified non-impactful per v1.3.6 CHANGELOG `Verified`). See per-CHANGELOG entries + `decisions/upstream_issues.md` for full per-issue closure narratives. |
