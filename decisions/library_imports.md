...
| `eval_toolkit.thresholds.TargetFPRSelector` | `scripts/fit_dual_policy_thresholds.py` (Phase 3 deliverable) | Detection-policy threshold fit on val per-(rung, fold, seed); FPR ≤ 1% target (per ADR-025) |
| `eval_toolkit.thresholds.TargetRecallSelector` | `scripts/fit_dual_policy_thresholds.py` (Phase 3 deliverable) | Verification-policy threshold fit on val per-(rung, fold, seed); recall ≥ 99% target (per ADR-025); honest infeasibility reporting via try/except wrap if selector raises on unreachable target |
