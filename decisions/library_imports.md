...
|---|---|---|
| `eval-toolkit` | `v1.0.2` (range `>=1.0.1,<2`) | `eval-toolkit>=1.0.1,<2` **(PyPI install per ADR-055; range pin per v1.0 stability-contract; v1.3.7 tightened lower bound to require module `eval_toolkit.audit_citation_alignment` shipped in v1.0.1)**. Trajectory: v0.31→v0.34 mid-Phase-4 X8; v0.34→v0.39 at v1.0.6; v0.39→v0.40 + git+https→PyPI at v1.0.8 (filed #44); v0.40→v0.42 at v1.0.9 (#44); v0.42→v0.43 at v1.2.8 (ledger only); v0.43→v0.44 at v1.2.14 (#50+#51); v0.44→v0.47 at v1.2.16 (skip-bumps; ledger); v0.47→v0.49→v0.50 at v1.3.5 (v0.50 SPEC 7 `rng=` adoption); v0.50→v1.0 at v1.3.6 (stability-contract opt-in); v1.0→v1.0.1→v1.0.2 at v1.3.7 (**consumed `eval_toolkit.audit_citation_alignment` — flat module per upstream ADR 0001; closes #73**; v1.0.2 adds #76 cleanup batch, no methodology change). See per-CHANGELOG entries + `decisions/upstream_issues.md` for full per-issue closure narratives. |
...
| Primitive | Imported in | Purpose |
|---|---|---|
| `eval_toolkit.audit_citation_alignment.validate_citations` + `ADRSubject` + `CitationMisalignment` + `extract_adr_subject_category` | `scripts/audit_citation_alignment.py::main` (v1.3.7 Action 1; SOFT CI gate at v1.3.7, promotes to HARD at v1.3.8) | Validate that "per ADR-NNN" citations in reader-facing markdown match the cited ADR's actual subject category. Catches the v1.3.2 P1-2 bug class (ADR-029 cited for reproducibility tier-lock where ADR-029 is actually test_markers). Consumer-side glue: builds `ADRSubject` map from `decisions/ADR-*.md` frontmatter (`title:` + `slug:` + inferred `category`); supplies the consumer's `CATEGORY_KEYWORDS` dict to the validator (11 categories at v1.3.7 seed: reproducibility/test_markers/cost/calibration/contamination/threshold/leakage/data/training/evaluation/reading_guide); globs reader-facing surfaces (top-level *.md/*.qmd + WRITEUP/*.md + docs/*.md) skipping CHANGELOG/SUBMISSION_AUDIT/decisions/transcripts. Upstream primitive shipped in v1.0.1 (flat module per upstream ADR 0001; closes eval-toolkit#73). |
