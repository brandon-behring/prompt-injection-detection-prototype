...
| `eval_toolkit.text_dedup.EmbeddingCosineStrategy.pairs_across` | `src/data/dedup.py::drop_train_test_leakage` (ADR-047 Commit 3) | Per-(candidate_train_val) top-1 max-cosine to test set; k=1 returns shape (n_query, 1) similarities + indices. Used in cosine-leakage layer (exact-hash layer stays as set-intersection project-specific layer per ADR-016 Q3) |
