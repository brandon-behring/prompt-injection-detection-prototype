...
| `eval_toolkit.plotting.plot_lift_ci` | `src/eval/figures.py::render_f6_lodo_breakdown` (Phase 4 Commit 4 per ADR-046 Q6) | F6 right panel — per-rung marginal CI visualization via library primitive |
| `eval_toolkit.plotting.save_figure` | `src/eval/figures.py` (Phase 4 Commit 4 export contract); `scripts/render_figures.py` (Phase 4 Commit 5 orchestrator) | Provenance-aware figure persistence per ADR-030 — writes `{stem}.meta.json` sidecar carrying figure_id + adr + git_commit_sha + timestamp_utc + matplotlib_version for every SVG/PDF/PNG output |
| `eval_toolkit.plotting.set_plot_style` + `PALETTE` | `src/eval/figures.py` (every renderer; Phase 4 Commit 4) | Consistent styling across the 7-figure slate per ADR-046 Q6; PALETTE's negative/positive/baseline/accent named colors used in F1 + F2 + F6 project-glue panels |
