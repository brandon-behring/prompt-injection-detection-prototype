...
superseded_by: ["055", "059"]  # narrow supersessions of the "git URL is the only viable spec format" sub-claim — once for eval-toolkit at v1.0.8 (ADR-055), once for runpod-deploy at v1.1.0 (ADR-059). research_toolkit retains git+https tag-pin per this ADR until it publishes to PyPI. Freeze policy + bump-triggers + tag-pin convention + uv.lock backstop all preserved.
references:
