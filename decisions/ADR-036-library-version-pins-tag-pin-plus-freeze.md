...
superseded_by: ["055"]  # narrow supersession: "git URL is the only viable spec format" sub-claim only; eval-toolkit moves to PyPI install at v1.0.8 (ADR-055). Freeze policy + bump-triggers + tag-pin convention + uv.lock backstop + runpod-deploy & research_toolkit git+https pins all preserved.
references:
