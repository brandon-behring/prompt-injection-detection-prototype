...
|---|---|---|---|
| Commit 1 | ADR-046 (Phase 4 implementation bundle; 6-commit cadence + scaffold-with-classical + always-emit-both-CIs auto-flag + MDE-on-every-emitted-CI + LLM-rater audit included per user override + library-first hybrid figures per project-wide invariant codification + Phase 5 prep deferred) + SPEC_SHEET §3.8 status table + SUBMISSION_AUDIT regen | n/a | **in progress** |
| Commit 2 | `src/eval/marginal_bootstrap.py` per ADR-022 (bootstrap_ci wrappers; 10K @ seed=1 headline + 10K @ seed=2 stability check) + `src/eval/cross_fold_ci.py` (cv_clt_ci headline per ADR-024) + `src/eval/mde.py` per ADR-006 (mde_from_ci wrappers + per-cell MDE matrix scaffolding) + smoke tests | `test_marginal_bootstrap_seed_stability` + `test_cv_clt_ci_methodology` (deferred-unskip at canonical evals run) | pending |
| Commit 3 | `src/eval/cross_fold_ci.py` extension — always-emit block-bootstrap-on-folds spoke per Q3 + A-008 + `a_008_flag_fired` auto-flag column when block_bootstrap_CI_halfwidth ÷ cv_clt_CI_halfwidth > 1.5; persisted to `evals/audit/cross_fold_ci_audit.parquet` per ADR-013 + smoke | `test_block_bootstrap_folds_spoke_present` + `test_a_008_flag_fired_when_ratio_exceeds_1_5` | pending |
| Commit 4 | `src/eval/figures.py` per Q6 — library-first hybrid 7-figure slate; consumes `eval_toolkit.plotting.{plot_pr_curve, plot_reliability_diagram, plot_bootstrap_distribution, plot_metric_bars, plot_lift_ci, save_figure, set_plot_style, PALETTE}` for F3 + F4 + F7-subpanels + project glue for F1 Pareto + F2 ROC + F5 slice heatmap + F6 LODO breakdown + F7 grid layout (cites upstream issues #14 + #15 + #16 as TODOs); SVG output to `docs/plots/` per ADR-030 + smoke | `test_figures_slate_7_svgs_present` + `test_save_figure_provenance_chunks_present` | pending |
| Commit 5 | Orchestration scripts — `scripts/run_marginal_bootstrap.py` per Q4 + `scripts/run_cv_clt_ci.py` per Q3 + `scripts/run_mde.py` per Q4 (sweeps ~100 cells across Phase 3 paired-bootstrap + Phase 4 marginal + cross-fold + op-point + ECE cells; emits `evals/audit/mde_per_cell.parquet`) + `scripts/render_figures.py` per Q6 (emits `docs/plots/F{1,2,3,4,5,6,7}.svg`) + `scripts/audit_reference_scorers.py` per Q5 user override (fires LLM-rater pass on ~50 disagreement-sampled pairs per reference rung; interactive approval per ADR-020 + ADR-045 Q4; ~$5 per A-002 envelope; persists to `evals/audit/reference_scorer_rater_audit.json`) + 5 subprocess-based smoke tests | smoke covers contract; canonical-data invariants deferred to operator-gated runs | pending |
| Commit 6 | Makefile Phase 4 targets (`marginal-bootstrap`, `cv-clt-ci`, `mde-battery`, `render-figures`, `audit-reference-scorers`) + extended `make smoke` (Phase 4 fixture-pipeline pass under ADR-027 sub-10-min budget) + `docs/ROADMAP.md` Phase 4 close note documenting `v0.9.0-rc1` rehearsal-tag dispatch per ADR-033 + Phase 5 unblock | n/a | pending |
