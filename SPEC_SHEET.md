...
- `bootstrap_ci` — per-metric finite-sample uncertainty. See eval-toolkit `bootstrap` methodology (see [README](https://github.com/brandon-behring/eval-toolkit#readme)).
- `paired_bootstrap_diff` — paired comparisons across rungs on the same test set. See eval-toolkit `comparison` methodology (see [README](https://github.com/brandon-behring/eval-toolkit#readme)).
- `mde_from_ci` — minimum detectable effect.
- Calibration battery (`reliability_curve`, `fit_temperature`, `fit_isotonic_calibrator`, ECE variants, Brier). See eval-toolkit `calibration` methodology (see [README](https://github.com/brandon-behring/eval-toolkit#readme)).
