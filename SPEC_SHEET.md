...
**Status**: `[LOCKED]` (Phase 0 through Phase 5 closed; 53 ADRs accepted across Phase 0-00 through v1.0.4 close (ADR-050 rung-slate narrowing + ADR-051 v1.0.x carryforward + ADR-052 full-FT OOD methodological reframing + ADR-053 reading-guide governance); every `[OPEN]` ledger row in SPEC_GREENFIELD locked to an ADR; remaining `[OPEN]` text references in this doc are documentation about the spec lifecycle convention, not unresolved decisions; rung-language alignment to ADR-050 narrowing landed at Item 7 of the v1.0.0 closure sweep; reading-guide headline-paragraph alignment to ADR-050 landed at v1.0.4)
