...
@pytest.mark.unit
@pytest.mark.skip(reason="historical Phase 4 stub superseded by ADR-062 F1-F5 slate")
def test_reviewer_figures_slate_f1_f5_svgs_present() -> None:
    """Reviewer-facing figure SVGs land at docs/plots/F{1..5}.svg per ADR-062 + ADR-030.

    ADR-046 originally implemented diagnostic F6/F7 paths, but ADR-062 narrowed
    the public reviewer-facing slate to F1-F5. The render_figures.py orchestrator
    emits the canonical public SVG files at docs/plots/F1.svg through F5.svg. This
    invariant asserts:
    (1) all 5 files exist under docs/plots/ after `make render-figures`;
    ...
    cross-link sanity.
    Skipped as a historical invariant stub; smoke tests cover current F1-F5 output.
    """
    raise NotImplementedError("invariant test stub — superseded by current F1-F5 smoke coverage")
...
def test_save_figure_provenance_chunks_present() -> None:
    ...
    with a provenance dict containing at minimum: figure_id (one of F1..F5),
    ...
    (1) every docs/plots/F{1..5}.svg contains the literal string "ADR-046";
