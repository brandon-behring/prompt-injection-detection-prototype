...
| §Tech-Stack | GPU class | H100 / A100 / consumer GPU / mixed | locked-to-runpod-deploy-0.7.7-gpu-failover-ladder (see ADR-020) | cross-references the §2 Model row 333 lock — same decision; ADR-020 encodes the 8-class gpu_order failover (H100/H200 tier 1 + A100-80G tier 2 + L40S tier 3 + A100-40G emergency) + dual-DC + BATCH_TABLE + flash-attn-2 fallback + dual-layer cost cap; rapid-ratification at Phase 0-08 Q3 — no new ADR per SDD one-ADR-per-decision discipline | ADR-020 | runpod-deploy docs; RunPod pricing |
| §Tech-Stack | Secrets management | env file / vault / cloud secret manager | locked-to-three-store-split-aligned-with-execution-context (see ADR-035) | three stores aligned with execution context — local .env (gitignored) + RunPod pod-secrets injected via runpod-deploy config + GH Actions repo Secrets; .env.example committed with placeholder values for HF_TOKEN + RUNPOD_API_KEY + OPENAI_API_KEY + ANTHROPIC_API_KEY; gitleaks pre-commit hook ratified as defense-in-depth; scripts/preflight_secrets.py Phase 1 work item; cloud secret manager (Doppler/Infisical/1Password/AWS/GCP) deferred as future-extension condition triggered by either production-grade scope extension or 5-plus additional secrets surfacing | ADR-035 | 12factor.net config; HF Hub auth docs; GH Actions Secrets docs; runpod-deploy patterns |
...
| §Tech-Stack | eval-toolkit version | semver pin | locked-to-v0.31.0-tag-pin-plus-freeze (see ADR-036) | tag pin to latest stable v0.31.0 at Phase 0-08 close; pyproject.toml dependencies specifier; freeze for submission window (Phase 0-08 close → v1.0.0 submission tag per ADR-033); bump-triggers limited to blocking upstream bug + critical security fix + post-submission reviewer-feedback patch per ADR-033 v1.0.x discipline; routine upstream releases NOT a bump trigger; uv.lock provides byte-level reproducibility on top | ADR-036 | https://github.com/brandon-behring/eval-toolkit/releases |
| §Tech-Stack | runpod-deploy version | semver pin | locked-to-v0.7.7-tag-pin-plus-freeze (see ADR-036; matches ADR-020 pre-existing pin) | tag pin to v0.7.7 matching ADR-020 lock; same tag-pin-plus-freeze policy as eval-toolkit per ADR-036; runpod-deploy v0.7.7 was already named in ADR-020 — Phase 0-08 records the pin in pyproject.toml | ADR-036 (matches ADR-020) | https://github.com/brandon-behring/runpod-deploy/releases |
| §Tech-Stack | research_toolkit version | semver pin | locked-to-v1.9.1-tag-pin-plus-freeze (see ADR-036) | tag pin to latest stable v1.9.1 at Phase 0-08 close; same tag-pin-plus-freeze policy per ADR-036; matches the toolkit version that produced the existing docs/research/ dossier per library_imports.md | ADR-036 | https://github.com/brandon-behring/research_toolkit/releases |
| §Tech-Stack | Python version pin | >=3.10 / >=3.11 / >=3.12 / >=3.13 | locked-to-requires-python-3-13 (see ADR-037) | ratify >=3.13 — existing state per prior commit bc8ce4e (.python-version=3.13 + pyproject requires-python=>=3.13); 3.13 in active support through Oct 2029 per PEP 602; uv handles installability transparently for any reviewer regardless of system Python; uv sync at Phase 0-08 close verifies all 3 load-bearing libraries install cleanly under >=3.13 | ADR-037 | https://devguide.python.org/versions/ |
...
| Roadmap | Phase tailoring | as-is / project-specific tailoring | locked-to-light-roadmap-edits-phase4-plus-phase5 (see ADR-038) | preserve kit-level 5-phase structure (Phase 1 Data + Phase 2 Training + Phase 3 Evaluation + Phase 4 Analysis + Phase 5 Writeup); tailor Phase 4 close to fire v0.9.0-rc1 rehearsal tag per ADR-033 + tailor Phase 5 description with Quarto + HF Hub + reproducibility expansions citing ADR-030 + ADR-031 + ADR-032 + ADR-033 + ADR-034; Phases 1-3 text preserved; rejected restructure options (Phase 4.5 / Phase 5a-b-c split / Phase 2b smoke-preflight / Phase 3+4 collapse) at Q6 walk | ADR-038 | inline §Roadmap; docs/ROADMAP.md |
...
| §1 Data | Source selection | dataset list | locked-to-Path-alpha (see ADR-016) | 4 train-positive sources + 2 train-benign sources (LMSYS + UltraChat) + 5 OOD slices; HarmBench + Tensor Trust + LLMail-Inject deferred to afterword | SPEC_SHEET §3.1 + configs/data/source_manifest.yaml | docs/research/datasets/; HF dataset cards |
| §1 Data | HF dataset revision pinning | pin SHAs / lockstep / `revision="main"` | locked-to-SHA-pin-manifest-documented-bumps (see ADR-016) | HF + git SHA pin at Phase 1 entry; manifest-documented bumps; ADR per bump only on schema change | configs/data/source_manifest.yaml | HF datasets revision docs |
...
| §1 Data | Benign subsample ceilings per source | open budget / per-source caps | locked-to-3K-pos-per-source-10K-benigns-per-source-seed-42 (see ADR-016) | mosscap + HackAPrompt capped at 3K each; deepset + Lakera-gandalf use-all post-dedup; LMSYS + UltraChat capped at 10K each; quality-filtered + attack-type-stratified + length-stratified in afterword | configs/data/source_manifest.yaml + SPEC_SHEET §3.1 | docs/research/datasets/; eval-toolkit splits.md (statistical power) |
...
| §6 Verify | Project-specific acceptance criteria | … | locked-to-six-gate-integration-checklist (see ADR-039) | 6-gate submission-readiness checklist aggregating across per-ADR acceptance_criterion fields — (1) zero [OPEN] in SPEC_SHEET; (2) zero open rows in SPEC_GREENFIELD ledger; (3) all 39+ invariant tests unskipped + green; (4) SUBMISSION_AUDIT clean (every claim Accepted or Superseded); (5) v0.9.0-rc1 rehearsal tag fired successfully before v1.0.0 per ADR-033; (6) all three reviewer URLs at v1.0.0 resolve; per-ADR criteria stay source of truth for granular gates; kit-default §6 gates preserved | ADR-039 | inline §6 |
| §6 Verify | Compute cost cap | strict / soft / none + dollar amount | locked-to-dual-layer-125-soft-plus-200-hard (see ADR-020) | dual-layer cap — per-job soft $125 via runpod-deploy budget.cost_cap_usd + project-wide hard $200 via scripts/cost_rollup.py CI gate; soft-cap-trigger flag at $80 cumulative; soft-cap-breach review at $125 cumulative documented in evals/cost_decisions.md; hard-cap breach requires superseding ADR; LLM-judge API spend tracked separately and rolled into the $200 project-wide hard cap; rapid-ratification at Phase 0-08 Q4 — no new ADR per SDD one-ADR-per-decision discipline | ADR-020 | RunPod pricing; runpod-deploy docs |
...
| §Kit-Ratify | Notebook format | jupytext-paired (default) / pure `.ipynb` / no notebooks | locked-to-jupytext-paired-illustrative-only (see ADR-013) | notebooks load precomputed artifacts; GPU runs are scripts | ADR-013 | `pyproject.toml [tool.jupytext.formats]`; `notebooks/README.md` |
